...
def get_first_difference_of_row(some_row: list[int]) -> list[int]:
    ...
    new_value_row_below = get_first_difference_of_row(new_list)[-1]
    value_to_add = new_list[-1] + new_value_row_below
...
meta_shred = [[int(y) for y in x.strip().split(' ')] for x in real_input if x .strip() != '']
